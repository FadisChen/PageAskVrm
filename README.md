...
PageAsk VRM 是一個 Chrome Manifest V3 Extension MVP：點擊工具列圖示後，會在目前網頁右下角注入透明的 `sha.vrm` Avatar。Avatar 可以使用 Gemini Live 進行語音對談，依回覆觸發情緒、動作與 lip sync；輸出文字可在設定中切換為 Avatar 上方的漫畫風格對白框。
...
6. 開啟一般 HTTPS 網頁，點擊工具列的 PageAsk VRM 圖示。
7. 點擊 Avatar 取得麥克風權限並開始對談。
